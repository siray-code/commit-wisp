...
Windows 安装脚本会把 `commit-wisp.exe` 放到当前目录，且不会修改 `PATH`。
...
Windows 中请使用 `.\commit-wisp.exe --dry-run`。
...
x64/arm64 脚本将 `commit-wisp.exe` 下载到当前目录，校验 `SHA256SUMS`，且不修改 `PATH`：

```powershell
irm https://raw.githubusercontent.com/siray-code/commit-wisp/main/scripts/install.ps1 | iex
```

请先进入目标目录，再运行脚本，之后使用 `.\commit-wisp.exe`。也可从 [GitHub Releases](https://github.com/siray-code/commit-wisp/releases) 手动下载并核对 checksum。
