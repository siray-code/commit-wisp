<p align="center">
  <img src="assets/logo.svg" alt="commit-wisp：沿着 Git 提交轨迹飞行的发光精灵" width="640">
</p>

<p align="center">
  从 Git 暂存区生成可审阅、可控制 token 预算的 AI 提交信息。
</p>
...
Windows PowerShell x64 / arm64 一键下载（将 `commit-wisp.exe` 放入当前目录，不修改 `PATH`）：

```powershell
irm https://raw.githubusercontent.com/siray-code/commit-wisp/main/scripts/install.ps1 | iex
```

Windows 脚本支持 x64 / arm64。请先进入希望存放 `commit-wisp.exe` 的目录再运行命令，下载完成后使用 `.\commit-wisp.exe`。脚本会校验 `SHA256SUMS`，且不会修改 `PATH` 或其他环境变量。
